...
const DashboardProfileEditor: React.FC<DashboardProfileEditorProps> = ({ userId }) => {
    ...
    const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            toast.error(err?.data?.message || t('failedUploadPicture'))
        }
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error(err?.data?.message || t('failedUpdateProfile'))
        }
    }
    ...
    if (error) {
        return (
            <div className="rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20">
                <p className="text-red-800 dark:text-red-200">{t('failedLoadProfile')}</p>
            </div>
        )
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div>
                        <label className="cursor-pointer">
                            <Button type="button" as="span" disabled={isUploading}>
                                {isUploading ? tc('uploading') : t('changePicture')}
                            </Button>
                            <input
                                type="file"
                                accept="image/*"
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                <h3 className="text-lg font-semibold mb-4">{t('professionalInformation')}</h3>
                <div className="grid gap-4 sm:grid-cols-2">
                    <div>
            ...
            {profile && (
                <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
                    <h3 className="text-lg font-semibold mb-4">{t('profileStats')}</h3>
                    <div className="grid gap-4 sm:grid-cols-3">
                        <div>
                            <p className="text-sm text-gray-600 dark:text-gray-400">{t('postsCount')}</p>
