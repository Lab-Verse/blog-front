  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4 rounded-lg border border-primary-200 bg-primary-50 px-4 py-3 dark:border-primary-800 dark:bg-primary-900/20">
          ...
          <div className="flex flex-1 gap-2">
            ...
            <Button
              ...
              className="bg-red-600 text-sm hover:bg-red-700"
            >
              Delete Selected
            </Button>
          </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {posts.length === 0 ? (
                  ...
                  posts.map((post) => (
                    <tr
                      key={post.id}
                      className={selectedPosts.includes(post.id) ? 'bg-primary-50 dark:bg-primary-900/10' : ''}
                    >
                      ...
                      <td className="relative py-5 pr-4 pl-3 text-right text-sm font-medium whitespace-nowrap sm:pr-0">
                        <div className="flex items-center justify-end gap-2">
                          <Link
                            href={`/dashboard/submit-post?id=${post.id}`}
      ...
      <Dialog open={showDeleteDialog} onClose={() => setShowDeleteDialog(false)}>
        <div className="p-6">
          <h3 className="mb-4 text-lg font-semibold">Delete Post</h3>
      ...
      <Dialog open={showBulkDeleteDialog} onClose={() => setShowBulkDeleteDialog(false)}>
        <div className="p-6">
          <h3 className="mb-4 text-lg font-semibold">Delete Multiple Posts</h3>
          <p className="mb-6 text-gray-600 dark:text-gray-400">
            Are you sure you want to delete {selectedPosts.length} posts? This action cannot be undone.
          </p>
          <div className="flex justify-end gap-3">
            <Button onClick={() => setShowBulkDeleteDialog(false)} disabled={bulkActionInProgress}>
              Cancel
            </Button>
